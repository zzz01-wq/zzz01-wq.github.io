...
let devModeEnabled=false,devErosionOverride=null,devCatOverride=null;
const ambientStartDay=3;
const devCatLabels={calm:'普通 / 安静',fed:'已喂食 / 饱足',hungry:'未喂食 / 饥饿','fur-loss':'掉毛 / 稀疏',abnormal:'异变 / 呼吸沉重','abnormal-bare':'严重异变 / 大片脱毛'};
function erosionScore(){if(devModeEnabled&&devErosionOverride!==null)return[10,40,80][devErosionOverride];return Math.min(100,Math.max(0,(state.day-1)*3+state.houseThreat*8+state.furLoss*5));}
...
function ensureAnomaly(){if(state.day<ambientStartDay){if(!(devModeEnabled&&state.anomaly?.debugForced))state.anomaly=null;return;}if(!state.anomaly||state.anomaly.day!==state.day)scheduleAnomaly();}
function inspectAnomaly(){const anomaly=state.anomaly;if(!anomaly||anomaly.seen||state.focus<=0)return;state.focus--;anomaly.seen=true;state.insight++;state.events.push('anomaly'+state.day);log('记录侵蚀：'+anomaly.title);finish(anomaly.after);window.AudioManager?.play('distortion');window.EventArt?.show({group:'ambient',id:anomaly.id,title:anomaly.title});}
function beatFor(day){return dailyBeats[day]||dailyBeats[30];}
function observeBeat(){const beat=beatFor(state.day);if(done('beat'+state.day)||state.focus<=0)return;state.focus--;state.insight++;state.events.push('beat'+state.day);log('记录异常：'+beat.title);finish(beat.after);window.AudioManager?.play('paper');window.EventArt?.show({group:'beat',id:state.day,title:beat.title});}
function catStatus(){if(devModeEnabled&&devCatOverride)return`调试：${devCatLabels[devCatOverride]}`;if(state.sanity<35)return'毛发正在脱落';if(state.furLoss>=3)return'毛发大片脱落，呼吸声很重';if(state.events.includes('event8')&&state.furLoss>0)return'毛发稀疏，呼吸声很重';if(state.furLoss>0)return'毛发变得稀疏';if(state.events.includes('event8'))return'呼吸声很重，仍在盯着你';if(state.lastFedDay===state.day)return'安心地蜷着身子';return'安静地望着你';}
function catSprite(){if(devModeEnabled&&devCatOverride)return devCatOverride;if(state.events.includes('event8')&&state.furLoss>=3)return'abnormal-bare';if(state.events.includes('event8'))return'abnormal';if(state.furLoss>0)return'fur-loss';if(state.lastFedDay===state.day)return'fed';if(state.day===1&&state.feedCount===0)return'calm';return'hungry';}
function updateCatSprite(){const sprite=catSprite();document.querySelectorAll('[data-cat-sprite]').forEach(node=>{node.hidden=node.dataset.catSprite!==sprite;});}
function dayBand(day){return day<=7?1:day<=14?2:day<=21?3:4;}
function updateRoomLayers(rank){const band=dayBand(state.day);document.querySelectorAll('[data-room-stage]').forEach(node=>{node.hidden=Number(node.dataset.roomStage)>rank;});document.querySelectorAll('[data-room-date]').forEach(node=>{node.hidden=Number(node.dataset.roomDate)>band;});}
function updateDevPanel(){const panel=$('#dev-panel');if(!panel)return;panel.hidden=!devModeEnabled;if(!devModeEnabled)return;const erosion=erosionStage(),anomaly=state.anomaly;$('#dev-readout').innerHTML=`<span>天数：${state.day}/30</span><span>阶段：${['上午','下午','夜晚'][state.phase]}</span><span>房间：${areaNames[state.area]||state.area}</span><span>侵蚀：${erosion.label} ${erosionScore()}</span><span>猫图：${catSprite()}</span><span>喂猫：${state.feedCount} 次</span><span>掉毛：${state.furLoss} 次</span><span>注意力：${state.focus}/2</span><span class="dev-wide">异常：${anomaly?`${anomaly.id}${anomaly.seen?'（已记录）':''}`:'无'}</span><span class="dev-wide">内容：${Object.keys(dailyBeats).length} 日常 · ${Object.keys(events).length} 固定 · ${ambientAnomalies.length} 随机 · ${Object.keys(devSpecialEvents).length} 特殊 · ${Object.keys(endings).length} 结局</span>`;const detail=$('#dev-state-detail');if(detail){const pendingLabel=state.ending?`已展示结局：${endings[state.ending]?.[0]||state.ending}`:state.pending?`待处理事件：${state.pending}`:'当前没有待处理的剧情事件';detail.textContent=`${pendingLabel}。上方摘要会随调试操作实时刷新。`;}}
...
function update(){ensureAnomaly();const erosion=erosionStage();state.sanity=Math.max(0,Math.min(100,state.sanity));$('#day').textContent=`第 ${String(state.day).padStart(2,'0')} 天`;$('#time').textContent=['上午 · 09:00','下午 · 15:00','夜晚 · 21:00'][state.phase];$('#daybar').style.width=state.day/30*100+'%';$('#sanity').innerHTML=`${state.sanity} <small>/ 100</small>`;$('#cat').textContent=catStatus();updateCatSprite();$('#cluecount').textContent=String(state.clues.length).padStart(2,'0');$('#scene').classList.toggle('night',state.phase===2);updateRoomLayers(erosion.rank);$('#mindbar').style.width=state.sanity+'%';$('.game').classList.toggle('danger',state.sanity<35);$('.game').classList.toggle('eroded',erosion.rank>=1);$('#care').textContent=state.lastFedDay===state.day?'今日已喂食':'今日尚未喂食';$('#focus').textContent=`注意力 ${state.focus}/2`;document.querySelectorAll('.paperpoint').forEach(b=>{b.hidden=state.events.includes('motherRead');});const weather=state.phase===2?'夜深':state.day%3?'阴':'短暂放晴';$('#weather').textContent=`十二月 · ${weather}`;$('#erosion').textContent=`屋况·${erosion.label}`;window.AudioManager?.updateMood({phase:state.phase,erosion:erosion.rank,sanity:state.sanity});updateDevPanel();save();}
function say(t,options,label='此刻',illustration=null){window.EventArt?.show(illustration); $('#narrative').textContent=t;$('#storytype').textContent=label;$('#choices').replaceChildren();$('.story-body').scrollTop=0;for(const [name,fn]of options){const b=document.createElement('button');b.textContent=name;b.onclick=()=>{if(!state.ending&&!b.disabled){b.disabled=true;fn();update();if(state.sanity<=0&&!state.ending)end('lost');}};$('#choices').append(b);}update();}
...
function doc(id){if(!state.clues.includes(id)){state.clues.push(id);log('发现了'+docs[id].title);}window.AudioManager?.play('paper');modal(`<div class="meta">${docs[id].meta}</div><h2>${docs[id].title}</h2>${docs[id].body}`,sourceNoteIds.has(id)?'note':'default');update();}
...
function act(a,result){
 ...
 if(a==='cat'){
  ...
  if(!fedToday)options.push(['喂水和罐头',()=>{state.lastFedDay=state.day;state.feedCount++;state.catMissedDays=0;state.catFood--;state.catCareDays++;state.catTrust=Math.min(10,state.catTrust+1);log('给猫猫喂水和罐头。');window.AudioManager?.play('catMew');if(state.day>=9&&state.events.includes('event8')&&!state.clues.includes('cat')){doc('cat');finish('猫猫慢慢吃完了面条。你从碗底抽出一张被浸湿的纸条。');}else finish('猫猫慢慢吃完了罐头，用额头轻轻碰了碰你的手。');}]);
  if(state.pet!==state.day)options.push(['轻轻抚摸',()=>{state.sanity+=5;state.catTrust=Math.min(10,state.catTrust+1);state.pet=state.day;window.AudioManager?.play('catPurr');finish('你在她耳后摸了两下就收回手。猫猫发出很轻的呼噜声。');}]);
 }
 if(a==='kitchen'){
 ...
 if(a==='wardrobe'){
  ...
  options=[['打开柜门',()=>{state.sanity-=45;state.houseThreat+=2;log('打开衣柜，失去了几段记忆。');window.AudioManager?.play('wardrobe');finish('里面的房间，和外面一模一样。你立刻把柜门关上，手心满是冷汗。');}]];
...
function escapeEvent(){window.AudioManager?.play('distortion');pending=true;state.pending='escape';say('日光落在柜边。黑丝在火焰中蜷缩，门外传来邻居拖住某个东西的声音。猫猫挡在门前，背部开始大片脱毛。',[['独自从楼梯离开，不回头',()=>{pending=false;state.pending=null;end('escape');}],['抱起猫猫一起离开',()=>{pending=false;state.pending=null;end('catfall');}],['留在家里等待',()=>{pending=false;state.pending=null;state.sanity-=10;back();}]],'抉择',{group:'special',id:'escape',title:'断开的影子'});}
...
function runEvent(){const e=events[state.day];window.AudioManager?.playEvent(state.day);pending=true;state.pending='daily';if(!state.events.includes('event'+state.day))state.events.push('event'+state.day);say(e[1],e[2].map((s,i)=>[s,()=>{pending=false;state.pending=null;const ok=i===e[3];state.sanity+=ok?3:-27;if(ok)state.insight++;else{state.houseThreat++;if(state.day===8)state.catTrust=Math.max(0,state.catTrust-2);}log(`${e[0]}：${s}。`);say(ok?'你没有跟随那句诱人的建议。很久以后，异常消失了。':'周围突然安静下来。你失去了一段记忆，醒来时已经站在床边。清醒程度大幅下降。',[['继续探索当前区域',back]],ok?'平安':'侵蚀',{group:'event',id:state.day,title:e[0]});}]),e[0],{group:'event',id:state.day,title:e[0]});}
function arrival(){window.AudioManager?.play('doorOpen');pending=true;state.pending='arrival';say(`第 30 天，上午九点。门外响起三声敲门。“我回来了。”你握着手机，猫猫安静地坐在你身边。${state.furLoss?'她的毛发比一个月前稀疏了一些，你不知道妈妈会不会注意到。':''}`,[['核对时间与妈妈的样子，再开门',()=>{pending=false;state.pending=null;end(state.items.includes('phone')&&state.sanity>=45&&state.feedCount>=29?'wait':'falsemother');}],['不打开门，继续寻找逃生办法',()=>{pending=false;state.pending=null;back();}]],'最后一天',{group:'special',id:'arrival',title:'上午九点'});}
...
function showEnd(){const e=endings[state.ending];modal(`${window.EventArt?.html({group:'ending',id:state.ending})||''}<div class="meta">${e[2]}</div><h2>${e[0]}</h2><p>${e[1]}</p><p class="ending">${e[3]}</p><button class="action" id="again">重新开始</button>`);$('#again').onclick=restart;}
...
const devRoomAliases={客厅:'living',卧室:'bedroom',厨房:'kitchen',门口:'door',衣柜:'wardrobe',镜子:'mirror',窗边:'window',窗户:'window',猫猫:'cat'};
const devStageAliases={浅:0,加深:1,严重:2,light:0,deep:1,severe:2};
const devCatAliases={普通:'calm',安静:'calm',饱足:'fed',喂食:'fed',饥饿:'hungry',掉毛:'fur-loss',稀疏:'fur-loss',异变:'abnormal',严重异变:'abnormal-bare'};
const devSpecialEvents={arrival:{group:'special',id:'arrival',day:30,title:'上午九点',text:'第 30 天上午九点，门外响起三声敲门。检查时间、妈妈和猫猫的状态。'},escape:{group:'special',id:'escape',day:'12+',title:'断开的影子',text:'满足逃离条件后，借日光与火焰烧断柜外黑丝，决定是否离开。'}};
const devEndingAliases={归家:'wait',等待:'wait',等待结局:'wait',逃离:'escape',逃离结局:'escape',迷失:'lost',迷失结局:'lost',陌生人:'catfall',怀中的陌生人:'catfall',迟来的拥抱:'falsemother',假妈妈:'falsemother'};
const devStageLabels=['浅','加深','严重'];
function devArt(group,id){return window.EventArt?.resolve?.({group,id})||null;}
function devEventCatalogue(){
 const daily=Object.entries(dailyBeats).map(([id,beat])=>({key:`beat:${id}`,group:'beat',id:Number(id),day:Number(id),title:beat.title,text:beat.text,after:beat.after,illustration:devArt('beat',Number(id)),source:beat}));
 const scheduled=Object.entries(events).map(([day,event])=>({key:`event:${day}`,group:'event',id:Number(day),day:Number(day),title:event[0],text:event[1],choices:event[2],correctChoice:event[2][event[3]],illustration:devArt('event',Number(day)),source:event}));
 const ambient=ambientAnomalies.map(anomaly=>({key:`ambient:${anomaly.id}`,group:'ambient',id:anomaly.id,day:`${ambientStartDay}–30`,area:anomaly.area,areaName:areaNames[anomaly.area],stage:anomaly.stage,stageLabel:devStageLabels[anomaly.stage],title:anomaly.title,text:anomaly.text,after:anomaly.after,illustration:devArt('ambient',anomaly.id),source:anomaly}));
 const special=Object.values(devSpecialEvents).map(event=>({...event,key:`special:${event.id}`,illustration:devArt('special',event.id),source:event}));
 return{daily,scheduled,ambient,special};
}
function devEventRows(){const catalogue=devEventCatalogue();return[...catalogue.daily,...catalogue.scheduled,...catalogue.ambient,...catalogue.special];}
function devEvents(){devRequire();const catalogue=devEventCatalogue();const rows=devEventRows();console.info(`[妈妈留的纸条] 事件清单：日常观察 ${catalogue.daily.length} 条，固定事件 ${catalogue.scheduled.length} 条，随机侵蚀 ${catalogue.ambient.length} 条，特殊事件 ${catalogue.special.length} 条。触发示例：dev.event('beat:1')`);if(typeof console.table==='function')console.table(rows.map(({key,group,id,day,areaName,stageLabel,title,illustration})=>({key,group,id,day,area:areaName||'',stage:stageLabel||'',title,illustration:illustration||'未配置'})));return catalogue;}
function devFindEvent(value){
 const rows=devEventRows();
 if(value&&typeof value==='object'){const group=value.group==='scheduled'?'event':value.group;return rows.find(row=>row.group===group&&String(row.id)===String(value.id));}
 if(typeof value==='number'&&Number.isInteger(value))return rows.find(row=>row.group==='beat'&&row.id===value);
 const query=String(value??'').trim();
 if(/^\d+$/.test(query))return rows.find(row=>row.group==='beat'&&row.id===Number(query));
 const beat=query.match(/^(?:beat|daily|observe):?(\d+)$/i);if(beat)return rows.find(row=>row.group==='beat'&&row.id===Number(beat[1]));
 const scheduled=query.match(/^(?:event|fixed|scheduled):?(\d+)$/i);if(scheduled)return rows.find(row=>row.group==='event'&&row.id===Number(scheduled[1]));
 const ambient=query.match(/^(?:ambient|anomaly):(.+)$/i);if(ambient)return rows.find(row=>row.group==='ambient'&&row.id===ambient[1]);
 const special=query.match(/^(?:special):(.+)$/i);if(special)return rows.find(row=>row.group==='special'&&row.id===special[1]);
 return rows.find(row=>row.key===query||`${row.group}:${row.id}`===query||String(row.id)===query||row.title===query);
}
function devStatus(){const erosion=erosionStage();return{enabled:devModeEnabled,day:state.day,phase:['上午','下午','夜晚'][state.phase],room:state.area,roomName:areaNames[state.area],erosion:erosion.label,erosionRank:erosion.rank,erosionScore:erosionScore(),catSprite:catSprite(),feedCount:state.feedCount,furLoss:state.furLoss,houseThreat:state.houseThreat,focus:state.focus,anomaly:state.anomaly?.id||null,anomalySeen:!!state.anomaly?.seen,ending:state.ending||null,pending:state.pending||null,content:{daily:Object.keys(dailyBeats).length,scheduled:Object.keys(events).length,ambient:ambientAnomalies.length,special:Object.keys(devSpecialEvents).length,endings:Object.keys(endings).length}};}
function devRender(){update();if(!pending&&!state.ending)back();return devStatus();}
function devEnable(){devModeEnabled=true;update();console.info('[妈妈留的纸条] 开发者模式已开启。输入 dev.help() 查看命令。');return devStatus();}
function devRequire(){if(!devModeEnabled)devEnable();}
function devDisable(){devErosionOverride=null;devCatOverride=null;devModeEnabled=false;update();return devStatus();}
function devSetDay(value){devRequire();const day=Number(value);if(!Number.isInteger(day)||day<1||day>30)throw new RangeError('日期必须是 1 到 30 的整数');state.day=day;state.phase=0;state.ending=null;pending=false;state.pending=null;state.anomaly=null;ensureAnomaly();return devRender();}
function devSetRoom(value){devRequire();const room=devRoomAliases[value]||value;if(!areaNames[room])throw new Error(`未知房间：${value}。可用：${Object.keys(areaNames).join('、')}`);state.ending=null;pending=false;state.pending=null;state.area=room;return devRender();}
function devSetErosion(value){devRequire();if(value===null||value===undefined||value==='clear'||value==='清除'){devErosionOverride=null;return devRender();}const rank=typeof value==='number'?value:devStageAliases[value];if(!Number.isInteger(rank)||rank<0||rank>2)throw new Error('侵蚀阶段可用：0/1/2、浅/加深/严重');devErosionOverride=rank;return devRender();}
function devSetCat(value){devRequire();if(value===null||value===undefined||value==='clear'||value==='清除'){devCatOverride=null;return devRender();}const sprite=devCatAliases[value]||value;if(!Object.prototype.hasOwnProperty.call(devCatLabels,sprite))throw new Error('猫猫切图可用：calm、fed、hungry、fur-loss、abnormal、abnormal-bare');devCatOverride=sprite;return devRender();}
function devSetAnomaly(value){devRequire();let anomaly;if(value&&typeof value==='object'&&value.id)anomaly=ambientAnomalies.find(a=>a.id===value.id);else{const query=String(value||'');anomaly=ambientAnomalies.find(a=>a.id===query||a.title===query||a.area===query||devRoomAliases[query]===a.area&&a.stage===erosionStage().rank);}if(!anomaly)throw new Error('未找到异常。可传入异常 id、标题或房间名');state.anomaly={...anomaly,day:state.day,seen:false,debugForced:true};return devRender();}
function devSetFeed(value){devRequire();const count=arguments.length?Number(value):state.feedCount+1;if(!Number.isInteger(count)||count<0||count>30)throw new RangeError('累计喂猫次数必须是 0 到 30 的整数');state.feedCount=count;state.lastFedDay=count?state.day:0;state.catMissedDays=0;return devRender();}
function devSetFur(value){devRequire();const loss=Number(value);if(!Number.isInteger(loss)||loss<0||loss>30)throw new RangeError('掉毛次数必须是 0 到 30 的整数');state.furLoss=loss;if(loss&&!state.events.includes('catFur'))state.events.push('catFur');if(!loss)state.events=state.events.filter(e=>e!=='catFur');return devRender();}
function devSetThreat(value){devRequire();const threat=Number(value);if(!Number.isInteger(threat)||threat<0||threat>10)throw new RangeError('房屋威胁值必须是 0 到 10 的整数');state.houseThreat=threat;return devRender();}
function devSetFocus(value){devRequire();const focus=Number(value);if(!Number.isInteger(focus)||focus<0||focus>2)throw new RangeError('注意力必须是 0 到 2 的整数');state.focus=focus;update();return devStatus();}
function devSetSanity(value){devRequire();const sanity=Number(value);if(!Number.isFinite(sanity)||sanity<0||sanity>100)throw new RangeError('清醒度必须是 0 到 100');state.sanity=sanity;update();return devStatus();}
function devClear(){devErosionOverride=null;devCatOverride=null;update();return devStatus();}
function devSnapshot(){return JSON.parse(JSON.stringify({state,debug:{enabled:devModeEnabled,erosionOverride:devErosionOverride,catOverride:devCatOverride}}));}
function devTriggerEvent(value){
 devRequire();
 const entry=devFindEvent(value);if(!entry)throw new Error('未找到事件。先运行 dev.events() 查看 key，例如 beat:1、event:2、ambient:living-damp、arrival');
 state.ending=null;pending=false;state.pending=null;
 if(entry.group==='beat'){
  state.day=entry.day;state.phase=0;state.area='living';state.focus=2;state.events=state.events.filter(id=>id!==`beat${entry.day}`);state.anomaly=null;observeBeat();
 }else if(entry.group==='event'){
  state.day=entry.day;state.phase=[2,12,17,22].includes(entry.day)?1:0;state.area='living';state.events=state.events.filter(id=>id!==`event${entry.day}`);runEvent();
 }else if(entry.group==='ambient'){
  state.day=Math.max(ambientStartDay,Math.min(30,state.day));state.phase=0;state.area=entry.area;state.focus=2;state.anomaly={...entry.source,day:state.day,seen:false,debugForced:true};devRender();
 }else if(entry.id==='arrival'){
  state.day=30;state.phase=0;state.area='door';arrival();
 }else{
  state.day=Math.max(12,state.day);state.phase=0;state.area='wardrobe';escapeEvent();
 }
 console.info(`[妈妈留的纸条] 已触发 ${entry.key}：${entry.title}`);return{...devStatus(),event:entry.key};
}
function devEndings(){devRequire();const list=Object.entries(endings).map(([id,ending])=>({id,title:ending[0],category:ending[2],summary:ending[1],hint:ending[3],illustration:devArt('ending',id)}));console.info(`[妈妈留的纸条] 结局清单：${list.length} 个。查看示例：dev.ending('wait')`);if(typeof console.table==='function')console.table(list);return list;}
function devResolveEnding(value){if(typeof value==='number'&&Number.isInteger(value))return Object.keys(endings)[value-1];const query=String(value??'').trim();return endings[query]?query:(devEndingAliases[query]||Object.keys(endings).find(id=>endings[id][0]===query));}
function devShowEnding(value){devRequire();const id=devResolveEnding(value);if(!id||!endings[id])throw new Error('未找到结局。先运行 dev.endings() 查看 id：wait、escape、lost、catfall、falsemother');pending=false;state.pending=null;state.ending=null;end(id);console.info(`[妈妈留的纸条] 已展示结局 ${id}：${endings[id][0]}`);return{...devStatus(),ending:id};}
function devSounds(){devRequire();const manager=window.AudioManager;const usedBy=manager?.eventSounds||{};const list=(manager?.manifest||[]).map(sound=>({...sound,usedBy:Object.entries(usedBy).filter(([,key])=>key===sound.key).map(([day])=>`第 ${day} 天固定事件`)}));console.info(`[妈妈留的纸条] 声音清单：${list.length} 项。播放示例：dev.sound('catMew')，停止：dev.sound('stop')`);if(typeof console.table==='function')console.table(list);if(manager?.status)console.info('[当前播放状态]',manager.status());return list;}
function devSound(value,multiplier=1){devRequire();if(value===undefined||value===null||value===''||value==='list'||value==='列表')return devSounds();const manager=window.AudioManager;if(value==='stop'||value==='停止'||value==='all-stop'){manager?.stopAll?.();return manager?.status?.()||{enabled:false,stopped:true};}if(!manager?.manifest)throw new Error('声音管理器尚未加载');const query=String(value).trim();const sound=manager.manifest.find(entry=>entry.key===query||entry.label===query);if(!sound)throw new Error(`未找到声音：${value}。先运行 dev.sounds() 查看 key`);const factor=Number(multiplier);if(!Number.isFinite(factor)||factor<=0||factor>4)throw new RangeError('音量倍率必须大于 0 且不超过 4');const played=manager.preview?manager.preview(sound.key,factor):manager.play?.(sound.key,factor);const result={...sound,played:!!played,status:manager.status?.()||null};console.info(`[妈妈留的纸条] ${sound.label||sound.key}：${played?'已播放':'播放失败'}`);return result;}
const devPanelState={tab:'state',eventGroup:'daily',eventKey:null,endingId:null,soundKey:null};
const devPanelEventGroups=[{key:'daily',label:'日常',group:'beat'},{key:'scheduled',label:'固定',group:'event'},{key:'ambient',label:'侵蚀',group:'ambient'},{key:'special',label:'特殊',group:'special'}];
function devPanelNode(parent,tag,text,className){const node=document.createElement(tag);if(className)node.className=className;if(text!==undefined)node.textContent=text;parent.append(node);return node;}
function devSetPanelTab(tab){if(!['state','events','endings','sounds'].includes(tab))return;devPanelState.tab=tab;document.querySelectorAll('[data-dev-tab]').forEach(button=>button.setAttribute('aria-selected',String(button.dataset.devTab===tab)));document.querySelectorAll('[data-dev-tab-panel]').forEach(panel=>{panel.hidden=panel.dataset.devTabPanel!==tab;});if(tab==='events')devRenderEventPanel();if(tab==='endings')devRenderEndingPanel();if(tab==='sounds')devRenderSoundPanel();}
function devRenderEventPanel(){const groupRoot=$('#dev-event-groups'),list=$('#dev-event-list'),detail=$('#dev-event-detail');if(!groupRoot||!list||!detail)return;const catalogue=devEventCatalogue();groupRoot.replaceChildren();for(const group of devPanelEventGroups){const entries=catalogue[group.key];const button=devPanelNode(groupRoot,'button',`${group.label} ${entries.length}`);button.type='button';button.className=devPanelState.eventGroup===group.key?'is-selected':'';button.onclick=()=>{devPanelState.eventGroup=group.key;devPanelState.eventKey=null;devRenderEventPanel();};}const entries=catalogue[devPanelState.eventGroup]||[];if(!entries.some(entry=>entry.key===devPanelState.eventKey))devPanelState.eventKey=entries[0]?.key||null;list.replaceChildren();for(const entry of entries){const row=devPanelNode(list,'button',undefined,entry.key===devPanelState.eventKey?'dev-list-row is-selected':'dev-list-row');row.type='button';row.setAttribute('aria-pressed',String(entry.key===devPanelState.eventKey));const main=devPanelNode(row,'span',`${entry.key} · ${entry.title}`,'dev-row-main');const meta=entry.group==='ambient'?`${entry.stageLabel} · ${entry.areaName}`:entry.day?`第 ${entry.day} 天`:'';devPanelNode(row,'span',meta,'dev-row-meta');row.onclick=()=>{devPanelState.eventKey=entry.key;devRenderEventPanel();};}detail.replaceChildren();const selected=entries.find(entry=>entry.key===devPanelState.eventKey);if(!selected){devPanelNode(detail,'p','暂无事件');return;}devPanelNode(detail,'h4',`${selected.key} · ${selected.title}`);if(selected.day)devPanelNode(detail,'p',`时间：${selected.day}${selected.areaName?` · 地点：${selected.areaName}`:''}${selected.stageLabel?` · 阶段：${selected.stageLabel}`:''}`,'dev-detail-meta');devPanelNode(detail,'p',selected.text);if(selected.after)devPanelNode(detail,'p',`结果：${selected.after}`);if(selected.choices)devPanelNode(detail,'p',`选项：${selected.choices.join(' / ')}`);if(selected.illustration)devPanelNode(detail,'p',`插画：${selected.illustration}`,'dev-detail-meta');const actions=devPanelNode(detail,'div',undefined,'dev-detail-actions');const trigger=devPanelNode(actions,'button','触发并查看');trigger.type='button';trigger.onclick=()=>devTriggerEvent(selected.key);}
function devRenderEndingPanel(){const listRoot=$('#dev-ending-list'),detail=$('#dev-ending-detail');if(!listRoot||!detail)return;const entries=Object.entries(endings).map(([id,ending])=>({id,title:ending[0],category:ending[2],summary:ending[1],hint:ending[3],illustration:devArt('ending',id)}));if(!entries.some(entry=>entry.id===devPanelState.endingId))devPanelState.endingId=entries[0]?.id||null;listRoot.replaceChildren();for(const entry of entries){const row=devPanelNode(listRoot,'button',undefined,entry.id===devPanelState.endingId?'dev-list-row is-selected':'dev-list-row');row.type='button';row.setAttribute('aria-pressed',String(entry.id===devPanelState.endingId));devPanelNode(row,'span',`${entry.id} · ${entry.title}`,'dev-row-main');devPanelNode(row,'span',entry.category,'dev-row-meta');row.onclick=()=>{devPanelState.endingId=entry.id;devRenderEndingPanel();};}detail.replaceChildren();const selected=entries.find(entry=>entry.id===devPanelState.endingId);if(!selected){devPanelNode(detail,'p','暂无结局');return;}devPanelNode(detail,'h4',`${selected.id} · ${selected.title}`);devPanelNode(detail,'p',selected.category,'dev-detail-meta');devPanelNode(detail,'p',selected.summary);devPanelNode(detail,'p',`提示：${selected.hint}`);if(selected.illustration)devPanelNode(detail,'p',`插画：${selected.illustration}`,'dev-detail-meta');const actions=devPanelNode(detail,'div',undefined,'dev-detail-actions');const trigger=devPanelNode(actions,'button','直接展示结局');trigger.type='button';trigger.onclick=()=>devShowEnding(selected.id);}
function devSoundRows(){const manager=window.AudioManager;const usedBy=manager?.eventSounds||{};return(manager?.manifest||[]).map(sound=>({...sound,usedBy:Object.entries(usedBy).filter(([,key])=>key===sound.key).map(([day])=>`第 ${day} 天固定事件`)}));}
function devRenderSoundPanel(){const listRoot=$('#dev-sound-list'),detail=$('#dev-sound-detail');if(!listRoot||!detail)return;const entries=devSoundRows();if(!entries.length){listRoot.replaceChildren();detail.replaceChildren();devPanelNode(detail,'p','声音管理器尚未加载');return;}if(!entries.some(entry=>entry.key===devPanelState.soundKey))devPanelState.soundKey=entries[0].key;listRoot.replaceChildren();for(const entry of entries){const row=devPanelNode(listRoot,'button',undefined,entry.key===devPanelState.soundKey?'dev-list-row is-selected':'dev-list-row');row.type='button';row.setAttribute('aria-pressed',String(entry.key===devPanelState.soundKey));devPanelNode(row,'span',`${entry.key} · ${entry.label||entry.key}`,'dev-row-main');devPanelNode(row,'span',entry.loop?'循环':'单次','dev-row-meta');row.onclick=()=>{devPanelState.soundKey=entry.key;devRenderSoundPanel();};}detail.replaceChildren();const selected=entries.find(entry=>entry.key===devPanelState.soundKey);if(!selected)return;devPanelNode(detail,'h4',`${selected.key} · ${selected.label||selected.key}`);devPanelNode(detail,'p',`${selected.loop?'循环播放':'单次播放'} · 默认音量 ${selected.volume??'自动'}`,'dev-detail-meta');devPanelNode(detail,'p',`文件：${selected.src}`);devPanelNode(detail,'p',selected.usedBy?.length?`使用场景：${selected.usedBy.join('、')}`:'使用场景：普通交互或状态提示');const actions=devPanelNode(detail,'div',undefined,'dev-detail-actions');const trigger=devPanelNode(actions,'button','播放');trigger.type='button';trigger.onclick=()=>{devSound(selected.key);devRenderSoundPanel();};}
function setupDevPanel(){if(!$('#dev-panel'))return;document.querySelectorAll('[data-dev-tab]').forEach(button=>{button.onclick=()=>devSetPanelTab(button.dataset.devTab);});$('#dev-stop-sounds').onclick=()=>{window.AudioManager?.stopAll?.();devRenderSoundPanel();};devSetPanelTab('state');}
function devHelp(){const help='开发者模式命令：\n  devMode()                 开启并显示调试面板\n  dev.events()              列出 30 条日常、7 条固定、24 条随机、2 条特殊事件\n  dev.event("beat:1")        触发日常观察事件；也可用 event:2 / ambient:living-damp / arrival\n  dev.endings()              列出全部结局及插画\n  dev.ending("wait")         直接展示指定结局；也可传 1 到 5\n  dev.sounds()               列出全部声音、循环状态和使用场景\n  dev.sound("catMew")        播放指定声音；dev.sound("stop") 停止全部声音\n  dev.day(22)               跳到指定日期\n  dev.room("wardrobe")      切换房间\n  dev.erosion("严重")       强制屋况：浅 / 加深 / 严重\n  dev.cat("abnormal-bare")  强制猫猫切图\n  dev.anomaly("wardrobe-thread") 强制当天异常\n  dev.feed(29)              设置累计喂猫次数\n  dev.fur(3)                设置累计掉毛次数\n  dev.threat(2)             设置房屋威胁\n  dev.focus(2)              设置注意力\n  dev.sanity(100)           设置清醒度\n  dev.status()              查看当前状态\n  dev.snapshot()            导出完整状态\n  dev.clear()               清除视觉强制状态\n  devMode(false)            关闭开发者模式';console.info(help);return help;}
const devAPI={enable:devEnable,disable:devDisable,help:devHelp,status:devStatus,snapshot:devSnapshot,events:devEvents,event:devTriggerEvent,endings:devEndings,ending:devShowEnding,sounds:devSounds,sound:devSound,audio:devSounds,day:devSetDay,room:devSetRoom,erosion:devSetErosion,cat:devSetCat,anomaly:devSetAnomaly,feed:devSetFeed,fur:devSetFur,threat:devSetThreat,focus:devSetFocus,sanity:devSetSanity,clear:devClear};
function devMode(enabled=true){return enabled===false?devDisable():devEnable();}
window.devMode=devMode;window.dev=devAPI;window.mothersNoteDev=devAPI;
setupDevPanel();
...
$('#restart').onclick=restart;$('#help').onclick=()=>modal('<div class="meta">规则怪谈 · 交互小说</div><h2>在这里活下去</h2><p>点击场景中的物体或下方地点，检查物品、发现纸条。通过客厅的“继续这一天”推进上午、下午与夜晚；夜晚休息进入下一天。探索本身不消耗时间。</p><p>每天记得吃饭、喂猫，并找到可靠的计时工具。喂猫次数会累计；漏喂一晚会让猫猫掉毛，并留下越来越明显的异变。客厅每天有两点注意力，可以用来观察当天的异常，或整理客厅降低房屋威胁；注意力会在新的一天恢复。异常事件出现时，请依据线索选择行动。清醒降到零，便会迷失。</p><p>部分文字会随着日期和事件逐步显现：日记至少要等到第 3 天，门口便签要等到第 12 天，猫猫纸条会在经历猫猫异常后的下一次照顾中出现。你可以等到第 30 天；等待结局需要累计喂猫至少 29 次，也可以在第 12 天之后准备一条更早离开的路。</p><p>进度保存在当前浏览器。声音默认关闭。</p><p class="meta">根据用户提供的《妈妈留的纸条》及作者 QA 改编。压缩了部分文字，事件日期与具体逃生仪式为游戏创作。含心理恐怖，无突发跳脸。故事中的危险指令仅属于虚构情境。</p>');
function syncSoundButton(){const enabled=window.AudioManager?.isEnabled?.()??false;$('#sound').innerHTML=enabled?'声音 开 <span>◉</span>':'声音 关 <span>◌</span>';}
$('#sound').onclick=()=>{if(!window.AudioManager){$('#sound').textContent='声音不可用';return;}const enabled=window.AudioManager.toggle();$('#sound').innerHTML=enabled?'声音 开 <span>◉</span>':'声音 关 <span>◌</span>';};
syncSoundButton();
